...
-This is sort of the ideal setting, where the three regressors don't have anything to do with one another; the standard deviation across all simulations is more or less equal
-Thus, the variance inflation by including extra variables x2 and x3 was negligable 
...
- Now regressors depend heavily on x1
- What happens to the variability and estimated coefficients? HUGE variance inflation
- This goes to the general rule that if the variable you include is highly correlated to the thing you're interested in, you're going to inflate the variance more
- So, need to concern yourself with putting highly correlated variabels into your regression model unnecessarily; if diastolic blood pressure is in the model and you add systolic blood pressure - basically the same thing/very correlated - it will increase the standard error of diastolic blood pressure
- Similarly, if a model were to incorporate both height and weight (generally highly correlated) but necessary, may need to bite the bullet and deal with the extra standard error - but if it's unnecessary DON'T INCLUDE IT
- Main point: the more correlated the covariants are to the regressors that you're interested in, the worse off you're going to be in terms of paying a penalty for increased standard deviation
- If we omit variables that are important, we get bias. If we include variables that are unnecessary, we inflate standard errors. Especially if we include variables that are unnecessary that are correlated with our variable of interest
...
- Looking at first line, Agriculture value is ~2, meaning the standard error for the agriculture effect is double of what it would be if it were orthogonal to all the other regressors
- Others are quite high (Examination, Education) because they are highly related


```{r}
sqrt(vif(fit)) #I prefer sd 
```
- These values reflect the increase in Standard Deviation
...
- Res.Df: Residual Degrees of Freedom - number of data points minus number of parameters
- RSS: Residual Sums of Squares
- Df: Excess degrees of freedom - degrees of freedome between models
- Sum of Sq: Sum of Squares
- F: F Statistic

- What the P value tells you is that the inclusion of the other parameters appear to be necessary beyond just the parameters included in model 1 and model 2
- 
