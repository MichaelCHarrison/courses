...
---
```{r, fig.height = 5, fig.width = 5, cache = T}
...
```{r, fig.height = 5, fig.width=5, echo = FALSE, results='hide', cache=TRUE }
...
* Do `?influence.measures` to see the full suite of influence measures in stats. The measures include
  ...
  * `hatvalues` - measures of leverage; acceptional for checking data entry errors 
...
- Correlation suggested solely due to the existence of the outlier; point cloud offers no pattern without outlier present 
...
```
- this first point which is the ten, ten point is or as the magnitude larger of a dfbeta then, the remaining point.

```{r}
round(hatvalues(fit)[1 : 10], 3)
```
-The hat value for this point is much larger than the hat values for the remaining points. The hat values have to be between zero and one. So and it's of course much larger than the other points. So if we are looking at these, we would obviously single this point out. 
